...
/** A resolved Pkl package coordinate — `package://<Host>/<name>@<version>` — and
  * the single source of truth for every string derived from it: the package uri,
  * the `name@version` base, the cache file names, the on-disk cache-entry dir,
  * and the content-version scheme. It replaces the handful of verbatim
  ...
  */
final case class PackageRef(name: String, version: String) {

  /** The `package://…` uri a `dependencies` block / pin declares. */
  def uri: String = s"package://${PackageRef.Host}/$name@$version"

  /** `name@version` — the single path segment pkl's cache addresses a package by
    * (also the served artifact file's stem).
    */
...
object PackageRef {
  ...
  /** Parse a `package://<host>/<name>@<version>` uri into its coordinate. The one
    * regex, replacing the verbatim per-name duplicates that used to live in
  ...
  /** The content-derived version scheme — `<base>-g<hash-prefix>` — owned in one
    * place ([[LibPackage.build]] and [[DumpPackage.build]] both mint through
    * here). Same content bytes ⇒ same version everywhere.
    */
  def contentVersion(baseVersion: String, hash: String): String =
    s"$baseVersion-g${hash.take(12)}"

  /** The cache dir holding a package's artifacts, addressed by its `name@version`
    * base string (`package-2` is pkl's cache-format tag, observed on 0.31.1). The
    * one definition of the `package-2/<host>/<base>` layout — absorbed both
    * `LibPackage.entryDir` and the re-inlined copy in
