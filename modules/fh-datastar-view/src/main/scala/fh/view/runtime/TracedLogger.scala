...
/** The link between the two halves of #75: every log line written inside a span
  * carries that span's ids.
  *
  ...
  *
  * Why not SLF4J's MDC, which logback would pick up with no wrapper at all: MDC
  * is thread-local, and a cats-effect fiber moves between threads freely. A
...
final class TracedLogger(
  ...
  def trace(message: => String): IO[Unit] =
    trace(Map.empty[String, String])(message)
  def debug(message: => String): IO[Unit] =
    debug(Map.empty[String, String])(message)
  def info(message: => String): IO[Unit] =
    info(Map.empty[String, String])(message)
  def warn(message: => String): IO[Unit] =
    warn(Map.empty[String, String])(message)
  def error(message: => String): IO[Unit] =
    error(Map.empty[String, String])(message)
