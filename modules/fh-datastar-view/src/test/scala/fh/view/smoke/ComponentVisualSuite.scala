...
/** "Verify looks": component-level screenshots of [[SmokeDashboard]] against
  ...
  * fundamentally can't cover (correct HTML + correct CSS selectors says
  * nothing about whether the CSS actually paints right). Deliberately at
  * COMPONENT granularity, not one full-page screenshot per test: a failure
  * names the exact card that regressed, and one card's incidental reflow
  * doesn't fail every other card's snapshot.
  */
class ComponentVisualSuite extends SmokeSuite {
  ...
  test("entityCard (on) looks right") {
    withPage(SmokeDashboard.dashboard, HouseFixture.all, viewport) { (page, _) =>
      IO.blocking {
        settle(page)
  ...
  test("button looks right") {
    withPage(SmokeDashboard.dashboard, HouseFixture.all, viewport) { (page, _) =>
      IO.blocking {
        settle(page)
        VisualSnapshot.check("button", page.getByText("Toggle Kitchen").screenshot())
      }
    }
  }
  ...
  test("tabs bar + default panel look right") {
    withPage(SmokeDashboard.dashboard, HouseFixture.all, viewport) { (page, _) =>
      IO.blocking {
        ...
        val tabsCell = page
          .locator(".fh-cell", new Page.LocatorOptions().setHas(page.locator(".tabs")))
          .last()
  ...
  private def kitchenCard(page: Page) =
    page.locator("article.entity", new Page.LocatorOptions().setHasText("Kitchen"))
}
