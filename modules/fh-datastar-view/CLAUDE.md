...
| `src/js/` + `package.json` + `vite.config.ts` | The frontend, bundled by vite into MANAGED resources (`project/NpmPlugin.scala`, `frontendInstall`/`frontendBundle` — a `resourceGenerators` entry, so a plain compile builds it and **node + npm are a build requirement**). Three bundles, three shapes, hence three `vite build --mode` runs: `shell.ts` -> `fh/shell.js`, an IIFE inlined into every page by `Server.page` (classic script — `fhConn` is called mid-body and `fhUrl` by Datastar's first effect, so a deferred module is too late); `editor/app.js` -> `editor/app.js`, an ES module with CodeMirror + lsp-client bundled IN (no vendor file, no CDN, no import map); `editor/overlay.js` -> `editor/overlay.js`, an IIFE. The output tree mirrors the classpath, so the editor's built JS sits beside the hand-written `resources/editor/{index.html,app.css,overlay.css}`. Nothing built is committed; new code is TypeScript (`tsc --noEmit` runs as part of the build), the ported editor sources stay JS |
