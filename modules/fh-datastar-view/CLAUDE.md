...
| `resources/dashboards/lib/query.pkl` | The candidate-set query surface (`q.from(...).where(...).render(...)`), imported as `@fh-dashboard/query.pkl`. Folds registry conditions away at BUILD time (they select candidates) and emits only live ones as per-member guards, into the `SetNode`/`SetMember`/`SetClause` wire classes that live in `components.pkl`. **Plain Pkl stays the first answer** — a `for` over a typed dump list is still the right way to render a fixed set of lights; this earns its place only when membership must react to live state. Covers `where`/`orderBy`/`limit`/`caseOf`/`render`, aggregates (`count`/`any`/`none`/`all` — these are also what an `If` condition is built from), nested sets, and `q.entity(e)` for naming a DIFFERENT entity than the member. See `docs/plan-dynamics-one-entity-lifecycle.md` |
