...
| `resources/dashboards/lib/query.pkl` | The candidate-set query surface (`q.from(...).where(...).render(...)`), imported as `@fh-dashboard/query.pkl`. Folds registry conditions away at BUILD time (they select candidates) and emits only live ones as per-member guards, into the `SetNode`/`SetMember`/`SetClause` wire classes that live in `components.pkl`. **Plain Pkl stays the first answer** — a `for` over a typed dump list is still the right way to render a fixed set of lights; this earns its place only when membership must react to live state. Not yet here (phase 3): `limit`, ordering by a live value, aggregates, nested sets. See `docs/plan-dynamics-one-entity-lifecycle.md` |
