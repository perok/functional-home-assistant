...
- **The URL restores it** — `v.<declarer>.<name>` query params, read by `Server.varChoicesOf` at
  ...
  has no request to read them off again. The route (`POST /sse/var/…`) writes the session, and
  the control mirrors the COMMITTED value back into `v.` with `fhUrl`, as a tab bar does `ui.`.
