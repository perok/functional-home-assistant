...
- [0006 — Pkl as a second authoring language](0006-pkl-authoring-track.md) (additive `.pkl` track next to jsonnet: `SourceEval` extension dispatch, typed generated `lib/dump.pkl` over a hand-written `hass.pkl` schema, cards as classes, MVP subset with an explicit skip-list toward parity)
