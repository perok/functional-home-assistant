...
- **W17** — a cache across batches, `SoftReference`-held. (The batch-scoped memo it also named is
  no longer deferred: it landed with the per-variant digest, which is where it was actually needed.)
...
> a node's own rendering must contain no mount, its own or a descendant's

— and a third shape satisfies the proxy while failing the property: **a card that splices
`{{#children}}` into its `template` with no mount at all.** That is the pre-split container shape.
It has no mount, so `hasOwnRendering` says yes, so it is a log key and is rendered by id — and its
rendering carries its children, which may carry mounts.

Found by accident: the first fixture written for W18's test was exactly this shape, and the test
still failed after the fix. Rewriting it as a pure mount (what `Column` actually is now) made it
exercise the shipped shape and pass.

Two reasons it is not urgent. The shipped library has no such card — phase 1b turned every plain
container into a pure mount. And `validate`'s live-slot rule keys on `bareContainer`
...
| W17 | The **batch-scoped memo LANDED** — not with W13 (which needed no memo: `plan` already selects ids before `diff` renders them, so gating selection IS the laziness) but with the per-variant digest, where the verdict genuinely has to be computed once per variant and shared. Still deferred and measurement-gated: a longer-lived `(state, node, variant) -> HTML` cache across batches, `SoftReference`-held — a plain `WeakReference` entry dies at the next GC and would cache nothing | `Renderer` |
...
| W22 ✅ LANDED | **A variant is a log entry of its own.** `Fragment` holds a digest per variant (member index; 0 for everything that does not vary), keyed by NodeId still — the variant in the KEY would put a non-node-id into the ledger. The verdict is lazy and memoised per variant, so a variant nobody holds is never rendered and two viewers on one variant both get the patch. Removes the suppression loss W21 introduced | `FragmentLog`, `Patches`, `Server`, `Memo` |
...
| **6 — what a fragment is** ✅ LANDED | W18 ✅, W19 ✅, W13 ✅, W20 ✅, W21 ✅, W22 ✅ | found by reviewing the finished result; W18 first because it defines what "own html" is, W19 next because it removes work rather than adding it, W21 last because nothing needs it yet |
