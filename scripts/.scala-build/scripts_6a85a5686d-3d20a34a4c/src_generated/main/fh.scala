...
// `.fh/machine.json` as `cacheDir` (base.pkl's `moduleCacheDir`).
  ...
  def targets(
    ...
    slugOpt match
      case Some(slug) if entries.size > 1 =>
        die(
  ...
  /** PUT the entry's SOURCE to the instance's own workspace
    * (`/edit/file/<slug>.pkl`, the route the /edit editor saves through), so
    * the instance owns and re-evaluates it — the dashboard survives a restart.
    *
    * Only the named entry travels: an entry importing a laptop-local `.pkl`
    * will be written but fail to build on the instance (its imports are not
    * there) — that is what plain `push` is for. A slug the instance did not
    * have at startup lands on disk but goes live only on its next restart; the
    * watcher reloads the entries it already knows.
    */
      ...
      for
        ...
        _ <-
          if !watch then run
            ...
            reporting(run) *>
              IO.println(s"watching *.pkl — ctrl-c to stop") *>
  ...
  /** Every `*.pkl` under the workspace, skipping dot-directories (`.fh`,
    * `.git`, `.scala-build`). Deliberately the whole tree rather than the
    * pushed entries' import graph: a workspace holds a handful of files, an
    * edit to any of them can change what an entry evaluates to, and re-scanning
    * per tick means a newly created file is watched without restarting.
    */
  def pklSources: IO[List[Path]] = IO.blocking {
    val root = Paths.get(".")
