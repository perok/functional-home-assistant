final class fh$_ {
  def args = fh_sc.args$
  def scriptPath =
    """/home/kanper/Dropbox/dev/functional-home-assistant/scripts/fh.sc"""
  /*<script>*/
...
// `.fh/machine.json` as `cacheDir` (base.pkl's `moduleCacheDir`).
  ...
  def watchPush(
    ...
    def affectedBy(
      ...
      ts.toList.filter(target =>
        // An entry whose imports could not be analyzed is treated as reached by
        // everything: a redundant push, never a missed one.
