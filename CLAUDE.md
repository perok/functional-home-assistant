...
- **Pkl authoring track (ADR 0006)**: dashboards can ALSO be authored in [Pkl](https://pkl-lang.org)
  ...
  same fetch as `dump.libsonnet` (both gitignored). The Pkl MVP covers containers/sectionTitle/
  entityCard/button/slider + expr/serviceTap/navigate; tabs, popups/surfaces, dynamic groups and
  `exprOf` are deferred — see ADR 0006's skip-list before extending.
