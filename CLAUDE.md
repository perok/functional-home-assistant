...
| `fh/view/build/DumpPackage.scala`, `scripts/fh` (repo root) | The laptop story (ADR 0010): the dump as a content-versioned package (`fh-home@1.0.0-g<hash>`, seeded on every dump write) + the `fh` scala-cli script (`init`/`pull`/`push`/`update`; Typelevel toolkit + decline + in-process pkl-core for resolve/eval, installed by curl from GitHub raw, `update` sha256-compares against the repo copy; needs only scala-cli) |
